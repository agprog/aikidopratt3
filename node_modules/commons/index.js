...
module.exports = {
	dateFormat       : function(value){
		try{
			 return [value.getFullYear(),("00"+(parseInt(value.getMonth())+1)).slice(-2),("00"+value.getDate()).slice(-2)].join('-');
		}catch(err){
			console.log(err);
			return "";
		}
	...
	contextCreate  : function(req,application){
		...
		/*! --- cree les scripts     --*/
		scripts: scripts_angular.concat(scripts_application).concat(scripts_common_angular)
		}
	...
	upload : function(file,upload_dir,test){
		...
		if(file.mimetype == 'application/pdf'){
			...
			imagemagick.convert([__uploaded__+'[1]','-resize','100x150',config.UPLOADS_DIR+upload_dir+'/thumbnails/'+filename+'.png'],function(err,stdout){
					if(err){
						console.log(err);
					}
				});
